...
int max9867_enable_playback(int stereo)
{
    ...
    if (err==E_NO_ERROR) {
        printf("max9867_enable_playback(): Set headphone mode HPMODE_STEREO_SE_CLICKLESS successfully\n");
    } else {
        printf("max9867_enable_playback(): Set headphone mode HPMODE_STEREO_SE_CLICKLESS failed: MXC_Error %d", err);
...
int max9867_init(mxc_i2c_regs_t *i2c_inst, int mclk, int controller)
{
    ...
    // try to enable everything in normal mode, not shutdown
