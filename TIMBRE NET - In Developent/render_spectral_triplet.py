"""
Render guitar, teacher-pretransient, and student-pretransient WAVs.

This verifies whether a distilled spectral UNet checkpoint is close enough to
the teacher's pre-transient spectral output before training the transient
student. It intentionally does not use realtime_twostage.py because that runner
may lag behind train_spectral.py's raw-log mask/residual semantics.
"""
...
def load_student(path: str, device: torch.device, args):
    ...
    for param in student.parameters():
        param.requires_grad_(False)
    return student, frame_size, hop_size, n_fft, base_ch, log_scale, log_floor, residual_clip, spectral_output
...
class SpectralTripletRenderer(torch.nn.Module):
    ...
    def forward(self, audio_frame: torch.Tensor):
        ...
        teacher_audio = self._istft(torch.polar(teacher_mag, phase), length=length)
        ...
        metrics["log_mag_l1"] = F.l1_loss(student_log_mag, teacher_log_mag).detach()
        return teacher_audio, student_audio, metrics
...
class RenderEngine:
    def __init__(self, renderer: SpectralTripletRenderer, device: torch.device):
        ...
        self.teacher_ring = np.zeros(self.frame_size, dtype=np.float32)
    ...
    def process_hop(self, in_hop: np.ndarray):
        ...
        teacher_frame, student_frame, metrics = self.renderer(self.buf)

        self.teacher_ring += teacher_frame.squeeze(0).detach().cpu().numpy().astype(np.float32)
        self.student_ring += student_frame.squeeze(0).detach().cpu().numpy().astype(np.float32)

        teacher_hop = self.teacher_ring[:self.hop_size].copy()
        student_hop = self.student_ring[:self.hop_size].copy()
        self.teacher_ring[:-self.hop_size] = self.teacher_ring[self.hop_size:]
        self.teacher_ring[-self.hop_size:] = 0.0
        self.student_ring[:-self.hop_size] = self.student_ring[self.hop_size:]
        self.student_ring[-self.hop_size:] = 0.0
        return teacher_hop, student_hop, metrics
...
def main():
    ...
    teacher = load_teacher(args.teacher_ckpt, device, frame_size, hop_size, n_fft)
    ...
    print(
        ...
        f"n_fft={n_fft}, base_ch={base_ch}, log_scale={log_scale}, "
    ...
    for i in tqdm(range(n_steps), unit="hop", ncols=72):
        ...
        teacher_hop, student_hop, metrics = engine.process_hop(audio[start:end])
        teacher_out[start:end] = teacher_hop
    ...
    if tail_hops:
        ...
        for _ in range(tail_hops):
            teacher_hop, student_hop, metrics = engine.process_hop(zero_hop)
            teacher_tail.append(teacher_hop)
            student_tail.append(student_hop)
            for key, value in metrics.items():
                metric_sums[key] += float(value.item())
        teacher_out = np.concatenate([teacher_out, np.concatenate(teacher_tail)])
    ...
    teacher_out = teacher_out[:orig_len]
    student_out = student_out[:orig_len]

    out_dir = Path(args.output_dir)
    out_dir.mkdir(parents=True, exist_ok=True)
    save_audio(out_dir / "01_guitar_input.wav", guitar_out)
    save_audio(out_dir / "02_teacher_pretransient.wav", teacher_out)
    save_audio(out_dir / "03_student_pretransient.wav", student_out)

    teacher_t = torch.from_numpy(teacher_out).to(device).unsqueeze(0)
    ...
    report = {
        ...
        **stats("teacher_pretransient", teacher_t),
        **stats("student_pretransient", student_t),
    }
    lines = [
