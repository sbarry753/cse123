...
class MLPDecoder(nn.Module):
    def __init__(
        self,
        input_size  = 2 + N_MFCC,       # f0 + loudness + mfcc
        ...
        self.head_f0_correction = nn.Linear(hidden_size // 2, 1)             # fine f0 adjustment
    ...
    def forward(self, feature_vec: torch.Tensor, f0: torch.Tensor):
        ...
        global_amp = torch.sigmoid(self.head_global_amp(h))              # (B, 1) in [0,1]
        ...
        # f0 with learned correction (guitar intonation ≠ piano tuning)
        f0_correction = torch.tanh(self.head_f0_correction(h)).squeeze(-1) * 10.0  # ±10 Hz
        f0_corrected  = f0 + f0_correction
