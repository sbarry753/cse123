...
CHECKPOINT_DIR = "checkpoints_residual"

SAMPLE_RATE = 48000

# 96 = 2 ms at 48 kHz. Change back to 128 if you want.
BLOCK_SIZE = 96
...
# Residual regularization: discourages the model from overwriting input too much
...
def find_latest_checkpoint(checkpoint_dir: str):
    ...
    candidates = sorted(checkpoint_dir.glob("residual_autoenc_epoch_*.pt"))
    if not candidates:
        return None
    ...
    torch.save(
        {
            ...
            "model_type": "ResidualLiveCausalAutoencoder",
            "base_channels": 64,
        },
        path,
    )
...
def load_checkpoint(path: str, model: nn.Module, optimizer=None, scheduler=None, device=DEVICE):
    ...
    return start_epoch, best_loss, ckpt
...
class ResidualLiveCausalAutoencoder(nn.Module):
    """
    input waveform + onset envelope -> residual correction for latest block
    final output = input_block + residual_gain * residual_block
    """
    ...
    def __init__(self, base_ch=64):
        ...
        # Learnable wet amount for residual contribution
    ...
    def forward(self, x, onset, h0=None, return_state=False, return_residual=False):
        ...
        input_block = x[:, :, -BLOCK_SIZE:]
        residual_block = residual[:, :, -BLOCK_SIZE:]
...
def forward_unrolled(model, x_window, onset_window):
    ...
    for step in range(UNROLL_STEPS):
        ...
        preds.append(y_step)
        residuals.append(r_step)

        if (step + 1) % TBPTT_DETACH_EVERY == 0:
            h = h.detach()

    return torch.cat(preds, dim=-1), torch.cat(residuals, dim=-1)


# =========================================================
# Train
# =========================================================
def train(resume=True, checkpoint_path=None):
    ...
    model = ResidualLiveCausalAutoencoder(base_ch=64).to(DEVICE)
    ...
    if resume:
        ...
        if checkpoint_path is not None and os.path.exists(checkpoint_path):
            ...
            start_epoch, prev_loss, ckpt = load_checkpoint(
                checkpoint_path,
                model,
                optimizer=optimizer,
                scheduler=scheduler,
            ...
            if prev_loss is not None:
                print(f"  Previous avg loss: {prev_loss:.6f}")
            print()
        else:
            print("Resume requested, but no checkpoint found. Starting fresh.\n")

    if start_epoch > EPOCHS:
        print(f"Checkpoint is already at epoch {start_epoch - 1}, which is >= EPOCHS ({EPOCHS}).")
    ...
    for epoch in range(start_epoch, EPOCHS + 1):
        ...
        for batch_idx, (x, y, onset_env) in enumerate(loader):
            ...
            onset_env = onset_env.to(DEVICE, non_blocking=True)

            pred_chunk, residual_chunk = forward_unrolled(model, x, onset_env)
            target_chunk = y[:, :, CONTEXT_SAMPLES:]

            loss = total_loss_fn(pred_chunk, target_chunk, onset_env)
            loss = loss + RESIDUAL_L1_WEIGHT * residual_chunk.abs().mean()
            ...
            if (batch_idx + 1) % 10 == 0:
                wet = float(torch.clamp(model.residual_gain, 0.0, 1.0).item())
                print(
                    f"  [{epoch:03d}] batch {batch_idx + 1:>4}/{len(loader)} "
                    f"| loss={loss.item():.6f} | wet={wet:.3f}"
                )
        ...
        print(f"Epoch {epoch:03d}/{EPOCHS} | avg_loss={avg:.6f} | lr={lr:.2e} | wet={wet:.3f}")

        epoch_ckpt_path = os.path.join(CHECKPOINT_DIR, f"residual_autoenc_epoch_{epoch:03d}.pt")
        ...
        print(f"  Saved {epoch_ckpt_path}")
        print(f"  Updated {latest_ckpt_path}")
