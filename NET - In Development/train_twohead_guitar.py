...
def label_to_note_name(label_vec: torch.Tensor, meta: Dict[str, Any]) -> str:
    ...
    Uses meta["index_to_pitch"] if present (recommended).
    """
...
# ----------------------------
# This is a "wiring diagram" style view like your reference image:
#   - black background
#   - white node rings
#   - green/red edges based on sign of weights
#   - edge thickness/alpha based on |weights|
#   - node glow based on activation magnitude (forward hooks)
#   - right side: input waveform + top-k predicted notes
#
# NOTE: Conv layers are local in time, not fully-connected, but this
# visualization intentionally approximates "channel connectivity" as dense.
...
class LiveNetViz:
    ...
    What it shows:
      - Edges = current weights (aggregated over conv kernel)
      - Node glow = current activations (mean abs over time)
      - Input waveform = the current training sample window (10ms)
      - Top predictions = sigmoid(logits) top-k
      - TRUE note label = decoded from y_batch

    Performance:
      - Updating too often will slow training, so use --viz_every.
    """
    ...
    def __init__(
        ...
        # Pitch names for outputs (index -> "E2", etc)
        self.pitches = self.meta.get("index_to_pitch", None)

        # Expect your OnsetNet layout:
        ...
        # Node counts (we draw each conv output channel as a node)
        self.n_in = 8  # stylized input column (not literal 480 samples)
        ...
        # Activations captured by forward hooks (updated each viz step)
        self.act_c0 = None
        self.act_c1 = None
        self.act_c2 = None
        self.act_h  = None
        ...
        # network plot is pure drawing (no axes)
        self.ax_net.set_axis_off()

        # Build node positions and static artists
    ...
    def close(self):
        """Close the matplotlib window cleanly."""
        try:
            plt.close(self.fig)
        except Exception:
            pass
    ...
    def _install_hooks(self):
        """Forward hooks to grab activations for node glow."""
        def hook_c0(_, __, out):
            self.act_c0 = out.detach()
        def hook_c1(_, __, out):
            self.act_c1 = out.detach()
        def hook_c2(_, __, out):
            self.act_c2 = out.detach()
        def hook_pool(_, __, out):
            self.act_h = out.detach()
    ...
    def _make_positions(self):
        """
        Place nodes in 5 columns:
            input -> c0 -> c1 -> c2 -> out
        Coordinates are normalized [0..1] in axes space.
        """
        pos = {}
        cols = [
            ("in",  self.n_in),
        ...
        for (name, n), x in zip(cols, xs):
            if n <= 1:
                ...
                # spread nodes vertically with margins (0.1..0.9)
    ...
    def _build_static_artists(self):
        """
        Create:
          - node circles (white ring)
          - a dense set of edges between adjacent columns (light gray base)
        Then we update their color/thickness each step.
        """
        def add_node(key, radius=0.018):
            ...
            circ = Circle(
                (x, y),
                radius=radius,
                facecolor=(0, 0, 0, 1),     # black fill (gets green glow later)
                edgecolor=(1, 1, 1, 1),     # white ring
                linewidth=2.0,
            )
            self.ax_net.add_patch(circ)
            self.node_artists[key] = circ

        # Input column (stylized)
        for i in range(self.n_in):
            add_node(("in", i), radius=0.020)

        # Conv channels
        for i in range(self.n_c0):
            add_node(("c0", i), radius=0.018)
        for i in range(self.n_c1):
            add_node(("c1", i), radius=0.018)
        for i in range(self.n_c2):
            add_node(("c2", i), radius=0.018)

        # Outputs
        for i in range(self.n_out):
            add_node(("out", i), radius=0.016)
    ...
    def _add_dense_edges(self, a, b):
        """Create a line for every node pair between columns a and b."""
    ...
    def _get_act_vecs(self):
        """
        Convert activation tensors into per-channel magnitudes for node glow.
          act: (B,C,T) -> mean(|act|) over (B,T) -> (C,)
        """
        def chan_mag(t):
            if t is None:
                return None
        ...
        ah = None
        if self.act_h is not None:
            ah = self.act_h.detach().float().abs().mean(dim=(0, 2)).cpu()
        return a0, a1, a2, ah

    def _update_nodes(self):
        """Update node fill alpha based on activation magnitude (glow)."""
        a0, a1, a2, _ = self._get_act_vecs()
        ...
        def set_glow(layer, mags):
            ...
            for i in range(len(mags)):
                key = (layer, i)
                if key not in self.node_artists:
                    continue
                g = float(mags[i].item())
                # subtle green glow with alpha proportional to activation
    ...
    def _update_edges(self):
        """Update edge color/thickness/alpha from current weights."""
        ...
        for a, b, ka, kb, line in self.edge_artists:
            ia = ka[1]  # source index
            ib = kb[1]  # dest index

            if a == "in" and b == "c0":
                # conv0 has 1 input channel; input column is stylized
    ...
    def _update_wave_and_preds(self, x, logits, true_note: str):
        ...
        Update right-side panels:
          - waveform plot for current sample
          - top-k prediction bar plot
        """
        ...
        t = np.arange(len(x_np)) / float(self.sr) * 1000.0  # ms
        ...
        # Build human-readable labels (pitch names)
        labels = []
        for j in idxs:
        ...
        self.ax_pred.axvline(self.thresh, linestyle="--", linewidth=1.0)  # decision thresh
    ...
    def update(self, x_batch, y_batch, device):
        """
        Call during training. Runs a forward pass on ONE sample and updates visuals.
        We also decode y_batch[0] to show the true input note name.
        """
        self.model.eval()
        ...
        # Decode true label to a note name
...
def load_clips(dataset_root: str) -> Tuple[List[ClipInfo], int, int]:
    """
    Load:
      - metadata.json (sr, midi_vocab, index_to_pitch, etc.)
      - manifest.jsonl (paths to audio + label json)
      - per-clip label.json (multi_hot, onset_sample, etc.)
    """
    ...
    for it in manifest:
        ...
        # Labeled transient end sample (fallback: onset + 5ms)
        t_end = int(lab.get("transient_window_end", onset + int(0.005 * sr)))

        # Total number of samples in clip (optional; used for sampling bounds)
...
def train(args):
    ...
    try:
        for ep in range(1, args.epochs + 1):
            ...
            # New best => save best + export C header
            if f1 > best_score:
                best_score = f1
